...
require_once 'services/MyResearch/lib/SearchTerms.php';
 ...
 * @link     http://vufind.org/wiki/autocomplete Wiki
 */
class SearchTermsAutocomplete implements AutocompleteInterface
{
    /**
     * Constructor
     *
    ...
    public function getSuggestions($query)
    {
        ...
        $tag->whereadd("\"query\" LIKE lower('%{$query}%')");
        $tag->orderBy('count DESC');
        $tag->find();
        if ($tag->N) {
            while ($tag->fetch()) {
                if ($tag->query != '') {
