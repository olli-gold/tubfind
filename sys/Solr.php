        ...
        if (isset($_SESSION['shards']) && get_class($this) == 'Solr') {
            ...
            foreach ($_SESSION['shards'] as $current) {
                if (isset($configArray['IndexShards'][$current])) {
                    //check health of solr index before adding to shards
                     try {
                         #if ($this->_isIndexHealthy($configArray['IndexShards'][$current])) {
...
//        if (array_key_exists('xyz', $_SESSION) !== true || !isset($_SESSION)) {
...
//        }
    ...
    /**
     * 
     * Check index status using ping
     * @param string $index
     * @return boolean
     */
    private function _isIndexHealthy($index) {
        //check health of solr index before adding to shards
        $client = new Proxy_Request($index);
        $client->setMethod(HTTP_REQUEST_METHOD_GET);
        $client->setURL($index . "/admin/ping?wt=json");
    ...
    public function getRecord($id)
    {
        ...
        if (substr($id, 0, 2) === 'PC') {
            ...
            $fq = array(0 => 'showAll:"true"');
            $pcaddr = isset($configArray['IndexShards']['Primo Central']) ?
                "http://".$configArray['IndexShards']['Primo Central'] : null;
            $this->host = $pcaddr;
            $iniName = 'searches_primocentral';
            $searchSettings = getExtraConfigArray($iniName);
            if (isset($searchSettings['AuthorizedMode']['enabled'])) {
                if (substr($_SERVER['REMOTE_ADDR'], 0, strlen($searchSettings['AuthorizedMode']['ipRange'])) == $searchSettings['AuthorizedMode']['ipRange'] && 
                    $searchSettings['AuthorizedMode']['enabled'] != false && 
                    (($searchSettings['AuthorizedMode']['field'] && $searchSettings['AuthorizedMode']['value']) || $searchSettings['AuthorizedMode']['removeDefaultFilter'])) {
                        $authorized = true;
                }
                if ($authorized === true) {
                    if ($searchSettings['AuthorizedMode']['field'] && $searchSettings['AuthorizedMode']['value']) {
                        $fq = array();
                        $fq[] = $searchSettings['AuthorizedMode']['field'].':'.$searchSettings['AuthorizedMode']['value'];
                    }
                }
            }
        }
            ...
            $core = isset($configArray['Index']['default_core']) ?
                $configArray['Index']['default_core'] : null;
            $this->host = $idxaddr . '/' . $this->core;
        }

        // Query String Parameters
//        $options = array('q' => 'id:"' . addcslashes($id, '"') . '"', 'fq' => array(0 => 'showAll:"true"'));
        $options = array('q' => 'id:"' . addcslashes($id, '"') . '"', 'fq' => $fq);
    ...
    /**
     * Retrieves a document list containing a ID in the ppnlink field.
     *
     * @param string $id The document to retrieve from Solr
     *
     * @throws object    PEAR Error
     * @return array     The requested resources (or null if bad ID or no results)
     * @access public
     */
/*    public function getRecordsByPPNLink($id, $limit=100, $start=0)
    {
        if ($this->debug) {
            echo "<pre>Get Record: $id</pre>\n";
        }
        ...
        // Query String Parameters
        $options = array('q' => 'ppnlink:"' . addcslashes($id, '"') . '" AND collection_details:"GBV_ILN_23"', 'rows' => $limit, 'start' => $start);
        $result = $this->_select('GET', $options);
        if (PEAR::isError($result)) {
            PEAR::raiseError($result);
        }
...
*/
    ...
    public function getMoreLikeThis($id)
    {
        ...
        // Get a morelikethis Solr index if one is provided in config.ini:
        if (isset($configArray['Index']['morelikethis'])) {
            $this->host = $configArray['Index']['morelikethis'];
        }

        $additionalFilter = null;
        // Get a morelikethis filter if one is provided in config.ini:
        if (isset($configArray['Index']['morelikethisFilter'])) {
            $additionalFilter = $configArray['Index']['morelikethisFilter'];
        }
                ...
                if ($basic) {
                    foreach ($mungeOps as $operation) {
                        ...
                        case 'append':
                            ...
                            break;
                        case 'prepend':
                            $values[$mungeName] = $operation[1].$values[$mungeName];
                            break;
                        case 'lowercase':
    ...
    public function search($query, $handler = null, $filter = null, $start = 0,
        ...
        if ($cleanInput === true) $query = $this->cleanInput($query);
    ...
    private function _select($method = HTTP_REQUEST_METHOD_GET, $params = array(),
        ...
        if (array_key_exists('qt', $params) === false) {
            $params['qt'] = "";
        }
        ...
        if (is_array($this->_solrShards) && count($this->_solrShards) > 1 && $params['qt'] !== 'morelikethis') {
    ...
    {
        // Experiment mit Whitelisting
        /*
        $clean_string = preg_replace("/[^\w\d\s:*]/", "", $query);
        return $clean_string;
        */
        // Traditionelles Blacklisting
        $query = trim(str_replace($this->_illegal, ' ', $query));
        //Do not process obvious operators like AND, OR, NOT to lower case; they should be handled like operators if they are already in upper case
        $qArr = explode(' ', $query);
        $dontprocess = array('AND', 'OR', 'NOT');
        $newQ = array();
        foreach ($qArr as $term) {
            $invalidTerm = false;
            foreach ($this->_illegal as $ill) {
                if ($term == trim($ill)) {
                    $invalidTerm = true;
                }
            }
            if ($invalidTerm === true) continue;
            if (in_array($term, $dontprocess) || strstr($term, 'id:') !== false) {
                $newQ[] = $term;
            }
            else {
                $newQ[] = strtolower($term);
            }
        }
        $query = implode(' ', $newQ);
