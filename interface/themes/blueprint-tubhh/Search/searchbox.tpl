<div class="searchform">
  ...
  {else}
    <form method="get" action="{$path}/Search/Results" name="searchForm" id="searchForm" class="search">
      ...
      <input style="margin: 12px 6px 12px 12px;" type="radio" checked="checked" name="shard[]" value="GBV Central" /> Bücher <input type="radio" name="shard[]" value="Primo Central" /> Artikel | 
      {if $tab != "primo"}
      ...
      {if $shards}
        ...
        {foreach from=$shards key=shard item=isSelected}
          <input type="checkbox" {if $isSelected}checked="checked" {/if}name="shard[]" value='{$shard|escape}' /> 
          {if $shard|cat:'_hint'|translate != $shard|cat:'_hint'}
              <span title="{$shard|cat:'_hint'|translate}">{$shard|translate}</span>
          {else}
              {$shard|translate}
          {/if}
        {/foreach}
