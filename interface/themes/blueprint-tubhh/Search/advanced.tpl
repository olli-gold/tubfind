<div class="span-18">
  ...
  <form method="get" action="{$url}/Search/Results" id="advSearchForm" name="searchForm" class="search">
    <div class="advSearchContent">
      ...
      <input type="checkbox" name="localonly" value="1" checked="checked" />{translate text="Only locally available items"}<br/>

      <input type="submit" name="submit" value="{translate text="Find"}"/>
      {if $facetList}
        <h3>{translate text='Limit To'}</h3>
        {foreach from=$facetList item="list" key="label"}
        <div class="{if $label=='Sachgebiete' or $label=='Subjects'}span-10{else}span-4{/if}">
