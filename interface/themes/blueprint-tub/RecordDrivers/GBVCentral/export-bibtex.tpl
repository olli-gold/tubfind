...
{assign var=id value=$marcIdField->getData()}
{/if}
...
{/if}
{if $field|getvalue:'c'}
{assign var=yearIsSet value=true}
...
journal = {$bracket}{$marcField|getvalue:'t'}},
{/if}
{/if}
{assign var=marcField value=$marc->getField('952')}
{if $marcField}
{if $marcField|getvalue:'d'}
volume = {$bracket}{$marcField|getvalue:'d'}},
{/if}
{if $marcField|getvalue:'j' && $yearIsSet == false}
year = {$bracket}{$marcField|getvalue:'j'}},
{/if}
