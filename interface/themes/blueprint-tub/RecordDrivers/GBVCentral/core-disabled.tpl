...
{if empty($coreHasParent)} {if $coreSubtitle}{$coreSubtitle|escape}{/if} {/if}
{if $coreTitleSection}{$coreTitleSection|escape}{/if}
{if empty($coreHasParent)} {if $coreAltTitle} - {$coreAltTitle.0|escape}{/if} {/if}
...
<table cellpadding="2" cellspacing="0" border="0" class="citation" summary="{translate text='Bibliographic Details'}">
  ...
  {if !empty($coreHasParent)}
    <tr valign="top">
    <th>{translate text='Contained in'}: </th>
        <td>
            {foreach from=$coreTitleParent item=parentTitle name=parentloop}
                {$parentTitle|escape}<br>
            {/foreach}
        </td>
    </tr>
  {/if}
  ...
  <tr valign="top">
    ...
    <td>
      {foreach from=$coreURLs item=desc key=coreUrl name=loop}
        <a href="{if $proxy}{$proxy}/login?url={$coreUrl|escape:"url"}{else}{$coreUrl|escape}{/if}">{$desc|escape}</a><br/>
      {/foreach}
      {if $coreOpenURL}
  ...
  {if !empty($coreRemarks)}
    <tr valign="top">
        <th>{translate text='Remarks'}: </th>
        <td>
            {foreach from=$coreRemarks item=remark key=rId name=loop}
                {$remark|escape}<br/>
            {/foreach}
        </td>
    </tr>
