    ...
    {foreach from=$filterList item=filters key=field name="filterLoop"}
      {foreach from=$filters item=filter}
        <li data-icon="minus">
          <a data-icon="minus" rel="external" href="{$filter.removalUrl|escape}">{if !$smarty.foreach.filterLoop.first}{translate text="AND"} {/if}{translate text=$field}: {$filter.display|escape}</a>
        </li>
      {/foreach}
    {/foreach}
    </ul>
