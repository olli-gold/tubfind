...
{if !empty($coreMainAuthor)}
  ...
  <dd><a rel="external" href="{$path}/Search/Results?lookfor={$coreMainAuthor|escape:"url"}&type=Author&localonly=1">{$coreMainAuthor|escape}</a></dd>
{/if}
...
{if !empty($coreCorporateAuthor)}
  ...
  <dd>
    <p><a rel="external" href="{$path}/Search/Results?lookfor={$coreCorporateAuthor|escape:"url"}&type=Author&localonly=1">{$coreCorporateAuthor|escape}</a></p>
  </dd>
{/if}

{if !empty($coreContributors)}
  <dt>{translate text='Other Persons'}:</dt>
  <dd><p>
  {foreach from=$coreContributors item=field name=loop}
    <a rel="external" href="{$path}/Search/Results?lookfor={$field.name|escape:"url"}&type=Author&localonly=1">{$field.name|escape}</a> {if $field.function}({translate text=$field.function|escape}){/if}{if !$smarty.foreach.loop.last}<br/>{/if}
  {/foreach}
  </p></dd>
{/if}
